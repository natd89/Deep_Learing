...
image = tf.placeholder(tf.float32, shape=[10, im_size, im_size, 3], name='image')
eps = tf.placeholder(tf.float32, shape = [10, 1, 1, 1], name='eps')
...
z1 = np.random.uniform(size=100)
z2 = np.random.uniform(size=100)

num_steps = 10

z_array = np.array([slerp(z1, z2, t) for  t in  np.arange(0.0,1.0, 1.0/num_steps)])

im_array = sess.run(G, feed_dict = {z: z_array})
clip_neg = im_array<0
